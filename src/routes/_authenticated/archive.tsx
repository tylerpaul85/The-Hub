...
const ACCEPT =
  "image/png,image/jpeg,image/webp,application/pdf,.doc,.docx,.ppt,.pptx,.xls,.xlsx,.txt";
...
function ArchivePage() {
  ...
  const agents = useMemo(() => {
    ...
    items.forEach((i) => {
      if (i.agent_name) s.add(i.agent_name);
    });
  ...
  const campaigns = useMemo(() => {
    const s = new Set<string>();
    items.forEach((i) => {
      if (i.campaign_tag) s.add(i.campaign_tag);
    });
    return Array.from(s).sort();
  }, [items]);
  ...
  const filtered = useMemo(() => {
    ...
    let res = items.filter((i) => {
      if (q) {
        const hay = [i.title, i.notes, i.campaign_tag, i.agent_name, i.listing_address]
          .filter(Boolean)
          .join(" ")
          .toLowerCase();
    ...
    if (sort === "newest") res.sort((a, b) => (a.date_created < b.date_created ? 1 : -1));
      ...
      res.sort((a, b) => (a.agent_name ?? "").localeCompare(b.agent_name ?? ""));
    return res;
    ...
    <div className="p-6 space-y-6 max-w-[1400px] mx-auto">
      <header className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Searchable library of every finished asset.
          </p>
        </div>
        <div className="flex items-center gap-4">
          <div className="hidden sm:block text-xs text-muted-foreground">
            Storage used:{" "}
            <span className="font-medium text-foreground">{formatBytes(totalBytes)}</span> across{" "}
            {items.length} files
          </div>
      ...
      <div className="space-y-3">
        <div className="relative">
          ...
          <Input
            placeholder="Search title, notes, campaign, agent, listing…"
            value={search}
        ...
        <div className="flex flex-wrap gap-2 items-center">
          ...
          <FilterSelect
            ...
            options={ARCHIVE_PLATFORMS}
          />
          <FilterSelect
            value={brand}
            onChange={(v) => setBrand(v as "all" | Brand)}
      ...
      {filtered.length === 0 && !isLoading ? (
        <div className="border border-dashed border-border rounded-xl p-16 text-center text-muted-foreground">
          {hasFilters
            ...
            : "No assets yet. Upload your first piece to get started."}
        </div>
      ) : (
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
          {filtered.map((item) => (
            <ArchiveCard key={item.id} item={item} onClick={() => setDetailId(item.id)} />
          ))}
        </div>
      )}
      ...
      {uploadOpen && (
        <UploadDialog
          ...
          userId={user?.id ?? null}
          onDone={() => qc.invalidateQueries({ queryKey: ["archive"] })}
        />
      )}
      {detailId && (
        <DetailDialog id={detailId} open={true} onOpenChange={(o) => !o && setDetailId(null)} />
      )}
    </div>
  );
}
...
function ArchiveCard({ item, onClick }: { item: ArchiveItem; onClick: () => void }) {
  const { data: thumb } = useQuery({
    ...
    queryFn: async () =>
      item.file_path && isImage(item.file_type) ? signedUrl(item.file_path) : null,
  ...
  return (
    <button
      onClick={onClick}
      className="group text-left bg-card border border-border rounded-lg overflow-hidden hover:border-gold/60 transition-colors"
    >
          ...
          <div className="flex flex-wrap gap-1">
            {item.platforms.slice(0, 3).map((p) => (
              <Badge
                key={p}
                variant="outline"
                className={cn("text-[10px] px-1.5 py-0", PLATFORM_CHIP[p])}
              >
                {p}
              </Badge>
            ))}
...
function UploadDialog({
  ...
  const [form, setForm] = useState<{
    ...
    platforms: string[];
    brand: Brand;
    agent_mode: "member" | "freeform";
    agent_member_id: string;
    agent_freeform: string;
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      ...
      if (isVideo && !form.drive_url.trim())
        throw new Error("Google Drive URL required for videos");
      ...
      if (!isVideo && file) {
        ...
        if (file.type && !ALLOWED.has(file.type))
          throw new Error("File type not allowed. Use JPG, PNG, WEBP, PDF, or DOC/DOCX.");
        if (!/\.(jpe?g|png|webp|pdf|docx?)$/i.test(file.name))
      ...
      const agentName =
        form.agent_mode === "member"
          ? members.find((m) => m.id === form.agent_member_id)
            ? memberName(members.find((m) => m.id === form.agent_member_id)!)
            : null
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{isEdit ? "Edit asset" : "Upload to archive"}</DialogTitle>
        </DialogHeader>
        <form
          onSubmit={(e) => {
            e.preventDefault();
          ...
          {isVideo ? (
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                Videos are linked from Drive — not stored here.
              </p>
            </div>
          ) : (
            <div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <div className="flex gap-1 mt-1.5 mb-2">
                <Button
                  type="button"
                  size="sm"
                  variant={form.agent_mode === "member" ? "default" : "outline"}
                  onClick={() => setForm({ ...form, agent_mode: "member" })}
                >
                  Team member
                </Button>
                <Button
                  type="button"
                  size="sm"
                  variant={form.agent_mode === "freeform" ? "default" : "outline"}
                  onClick={() => setForm({ ...form, agent_mode: "freeform" })}
                >
                  Other
                </Button>
              </div>
              {form.agent_mode === "member" ? (
                <Select
                  value={form.agent_member_id}
                  onValueChange={(v) => setForm({ ...form, agent_member_id: v })}
                >
                  <SelectTrigger>
                ...
                <Input
                  value={form.agent_freeform}
                  onChange={(e) => setForm({ ...form, agent_freeform: e.target.value })}
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => setForm({ ...form, listing_address: e.target.value })}
...
function DetailDialog({
  ...
  const copyLink = async () => {
    if (item?.drive_url) {
      await navigator.clipboard.writeText(item.drive_url);
  ...
  if (editing && item) {
    return (
      <UploadDialog
        ...
        onDone={() => {
          qc.invalidateQueries({ queryKey: ["archive"] });
          qc.invalidateQueries({ queryKey: ["archive-item", id] });
        }}
        edit={item}
      />
    );
            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-wrap gap-2 pt-2 border-t border-border">
                {item.drive_url ? (
                  ...
                  <Button asChild>
                    <a href={previewUrl} download={item.title}>
                ...
                {user?.id === item.uploaded_by && (
                  <>
                    ...
                    <Button
                      ...
                      onClick={() => {
                        if (confirm("Delete this asset?")) del.mutate();
                      }}
                    >
