...
function LiveStatsDiagnostics() {
  ...
  const data = mutation.data as
    { fub: ProbeResult[]; sisu: ProbeResult[]; meta: Record<string, unknown> } | undefined;
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Test connections to Sisu and Follow Up Boss. Inspect raw responses to confirm available
            fields before designing the TV display.
          </p>
        </div>
        ...
        {results.map((r, i) => (
          <Card key={i} className="p-4 space-y-2">
            <div className="flex items-center justify-between gap-2 flex-wrap">
              ...
              {r.url && (
                <code className="text-[10px] text-muted-foreground break-all">{r.url}</code>
              )}
            </div>
            ...
            {r.contentType && (
              <div className="text-[10px] text-muted-foreground">content-type: {r.contentType}</div>
            )}
            {r.data !== undefined && (
