  ...
  const reopen = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await sb
        .from("issues")
        .update({ status: "open", outcome_note: null })
        .eq("id", id);
      if (error) throw error;
    },
    onSuccess: () => {
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Central list of all team issues. Resolve or delete from here or inside any L10 meeting.
          </p>
        </div>
      ...
      <Tabs value={tab} onValueChange={(v) => setTab(v as any)}>
        <TabsList>
          <TabsTrigger value="pending" className="gap-1.5">
            New{" "}
            <span className="text-[10px] px-1.5 py-0.5 rounded bg-gold/15 text-gold border border-gold/40">
              {counts.pending}
            </span>
          </TabsTrigger>
        ...
        {tab === "pending" && (
          <div className="mt-3 text-xs text-muted-foreground border-l-2 border-gold/60 pl-3">
            Headlines flagged as issues land here for review. Confirm to send to the active Issues
            list, or dismiss if it doesn't need solving.
          </div>
        )}
        ...
        <TabsContent value={tab} className="mt-4 space-y-2">
          ...
          {filtered.map((i) => {
            ...
            return (
              <div
                key={i.id}
                className={`border rounded-lg p-3 flex items-start gap-3 bg-card ${isPending ? "border-gold/40" : "border-border"}`}
              >
                  ...
                  {isPending && (
                    <>
                      <Button
                        size="sm"
                        variant="default"
                        className="h-8 bg-gold text-navy hover:bg-gold/90"
                        onClick={() => confirmPending.mutate(i.id)}
                      ...
                      <Button
                        ...
                        variant="outline"
                        className="h-8"
                        onClick={() => dismissPending.mutate(i.id)}
