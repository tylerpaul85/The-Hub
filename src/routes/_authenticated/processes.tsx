...
function ProcessesPage() {
  ...
  const { data: processes = [] } = useQuery<Process[]>({
    ...
    queryFn: async () => {
      const { data, error } = await sb
        .from("processes")
        .select(
          "id, title, category_id, content, steps, checklist_mode, last_updated_by, updated_at",
        )
  ...
  const { data: editorEmails = {} } = useQuery<Record<string, string>>({
    queryKey: [
      "process-editor-emails",
      processes
        .map((p) => p.last_updated_by)
        .filter(Boolean)
        .join(","),
    ],
    ...
    queryFn: async () => {
      const ids = Array.from(
        new Set(processes.map((p) => p.last_updated_by).filter(Boolean) as string[]),
      );
        ...
        <div className="p-3 border-b border-border space-y-2">
          <div className="flex items-center justify-between">
            <h1 className="font-semibold flex items-center gap-2">
              <FileText className="h-4 w-4 text-gold" /> Processes
            </h1>
            {isAdmin && (
              <Button
                size="sm"
      ...
      <div className="flex-1 overflow-auto">
        {selected ? (
          <ProcessDetail
            ...
            lastEditorEmail={
              selected.last_updated_by ? editorEmails[selected.last_updated_by] : undefined
            }
            isAdmin={isAdmin}
            ...
            onDelete={() => {
              if (confirm("Delete this process?")) deleteProcess.mutate(selected.id);
            }}
  ...
  const startRun = useMutation({
    mutationFn: async () => {
      const { data: run, error } = await sb
        .from("process_runs")
        .insert({ process_id: p.id, started_by: currentUserId })
        .select()
        .single();
      if (error) throw error;
        ...
        <div>
          ...
          <div className="text-xs text-muted-foreground mt-1">
            ...
            {lastEditorEmail && (
              <>
                {" "}
                by <span className="text-foreground/70">{lastEditorEmail}</span>
              </>
            )}
          </div>
        </div>
        ...
        <section className="bg-card border border-border rounded-lg p-4">
          <div className="flex items-center justify-between mb-3">
            ...
            {activeRunId && !activeRun?.run?.completed_at && (
              <Button size="sm" variant="outline" onClick={() => finishRun.mutate()}>
                Finish
              </Button>
            )}
          </div>
            ...
            <ul className="space-y-2 text-sm">
              {(activeRun?.steps ?? []).map((s: any) => (
                <li key={s.id} className="flex items-start gap-2">
                  ...
                  />
                  <div>
                    <div className={s.checked_at ? "line-through text-muted-foreground" : ""}>
                      {linkify(s.label)}
                    </div>
                    {s.checked_at && (
          ...
          <div className="flex items-center justify-between rounded-md border border-border p-3">
            <div>
              ...
              <div className="text-xs text-muted-foreground">
                Steps become checkable items users can run.
              </div>
            </div>
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-xl">
        <DialogHeader>
          <DialogTitle>Checklist Run History</DialogTitle>
        </DialogHeader>
        {runs.length === 0 ? (
            ...
            {runs.map((r: any) => (
              <div key={r.id} className="py-3">
                ...
                <div className="text-xs text-muted-foreground">
                  ...
                  {r.completed_at ? (
                    <> · Completed {format(new Date(r.completed_at), "MMM d, h:mm a")}</>
                  ) : (
                    <> · In progress</>
                  )}
                </div>
