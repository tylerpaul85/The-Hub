    ...
    <>
      ...
      >
        <img
          ...
          width={size}
          height={size}
          className="rounded border border-gold/20"
        />
      </button>
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-md">
          ...
          <div className="flex flex-col items-center gap-4 py-2">
            {bigSrc && (
              <img src={bigSrc} alt="QR code" className="w-full max-w-sm h-auto rounded" />
            )}
            <a
              href={url}
              target="_blank"
              rel="noopener noreferrer"
              className="text-xs text-gold hover:underline break-all text-center"
            >
              Open link
            </a>
          </div>
