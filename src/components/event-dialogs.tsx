...
export function NewEventDialog({
  ...
  const submit = async () => {
    ...
    try {
      ...
      const sugRows = SUGGESTION_OFFSETS.map((o) => ({
        event_id: ev.id,
        slot_type: o.type,
        suggested_date: addDays(date, o.days),
        status: "pending",
    ...
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Fill in the event details. Suggested content slots will be generated automatically.
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-3">
          ...
          {NEEDS_LISTING.includes(type) && (
            <div>
              <label className="text-xs text-muted-foreground">Linked listing address</label>
            ...
            <div className="max-h-32 overflow-y-auto border border-border rounded-md p-2 space-y-1">
              {members.map((m) => (
                <label key={m.id} className="flex items-center gap-2 text-sm cursor-pointer">
                  <Checkbox
                    checked={hosts.includes(m.id)}
                    onCheckedChange={() => toggleHost(m.id)}
                  />
                  {displayName(m)}
                </label>
              ))}
        ...
        <DialogFooter>
          ...
          <Button onClick={submit} disabled={saving} className="bg-gold text-navy hover:bg-gold/90">
...
export function EventDetailSheet({
  ...
  const addItem = async () => {
    ...
    const { error } = await sb
      .from("event_checklist_items")
      .insert({ event_id: event.id, label: newItem.trim(), sort_order: items.length });
    if (error) {
      toast.error(error.message);
      return;
    }
  ...
  const hostNames = event.hosts
    .map((id) => displayName(members.find((m) => m.id === id)))
    .filter(Boolean);
  ...
  const deleteEvent = async () => {
    ...
    if (
      !confirm(
        `Delete event "${event.name}"? This removes its checklist and content suggestions. Tasks linked to it will be unlinked, not deleted.`,
      )
    )
      return;
      ...
      <SheetContent className="w-full sm:max-w-xl overflow-y-auto">
        <SheetHeader>
          <div className="flex items-center justify-between gap-2">
            <Badge
              variant="outline"
              className={cn("text-[10px]", EVENT_TYPE_CLASS[event.type as EventType] ?? "")}
            >
              {event.type}
            </Badge>
            {isAdmin && (
          ...
          <div className="grid grid-cols-2 gap-3 text-xs">
            ...
            {event.headcount != null && (
              <div>
                ...
                <div>{event.headcount}</div>
              </div>
            )}
            {event.budget != null && (
              <div>
                <div className="text-muted-foreground">Budget</div>
                <div>${Number(event.budget).toLocaleString()}</div>
              </div>
            )}
            {event.linked_listing && (
              <div className="col-span-2">
                <div className="text-muted-foreground">Listing</div>
                <div>{event.linked_listing}</div>
              </div>
            )}
            {hostNames.length > 0 && (
              <div className="col-span-2">
                <div className="text-muted-foreground">Hosts / assigned</div>
                <div>{hostNames.join(", ")}</div>
              </div>
            )}
            {event.notes && (
              <div className="col-span-2">
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-xs text-gold">
                {tasksDone} of {eventTasks.length} done
              </span>
            </div>
            ...
            <div className="mt-3 rounded-md border border-border bg-muted/20 p-2.5 space-y-2">
              ...
              <div className="flex gap-2 flex-wrap">
                ...
                <Input
                  type="date"
                  value={qDue}
                  onChange={(e) => setQDue(e.target.value)}
                  className="w-[160px]"
                />
                <Button
          ...
          {suggestions.length > 0 && (
            <div>
              <div className="flex items-center gap-2 mb-2">
                ...
                <h3 className="font-semibold text-sm">Suggested Content</h3>
              </div>
              <div className="space-y-2">
                {suggestions.map((s) => (
                  <div
                    key={s.id}
                    className="flex items-center justify-between rounded-md border border-border bg-card/50 p-2.5"
                  >
                    <div>
                      <div className="text-sm font-medium">{s.slot_type}</div>
                      <div className="text-xs text-muted-foreground">
                        {format(new Date(s.suggested_date + "T00:00:00"), "EEE, MMM d")}
                      </div>
                    </div>
              ...
              {items.map((i) => (
                <div key={i.id} className="flex items-center gap-2 group">
                  ...
                  <span
                    className={cn(
                      "text-sm flex-1",
                      i.completed && "line-through text-muted-foreground",
                    )}
                  >
                    {i.label}
                  </span>
                  {i.completed && i.completed_by && (
                    <span className="text-[10px] text-muted-foreground">
                      {displayName(members.find((m) => m.id === i.completed_by))}
                    </span>
                  )}
                  {isAdmin && (
                    <Button
