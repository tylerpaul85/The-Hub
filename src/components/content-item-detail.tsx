...
type FormShape = {
  ...
  description: string;
  blog_content: string;
  blog_doc_link: string;
  youtube_thumbnail_url: string;
  youtube_video_title: string;
  email_subject_line: string;
  email_body: string;
  meta_media_link: string;
  ...
  const { data: history = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await (supabase as any)
        ...
        .select("id,field,old_value,new_value,user_id,created_at")
        ...
        <DialogContent className="max-w-md">
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={!revisionNoteDraft.trim()}
    ...
    <div className="rounded-md border border-border bg-card/30 p-3">
      <div className="flex items-center justify-between mb-2">
        <div className="text-sm font-semibold flex items-center gap-2">
          <ListChecks className="h-4 w-4 text-gold" /> Linked Tasks
        </div>
        {canEdit && (
      ...
      {(tasks as any[]).length === 0 ? (
        <div className="text-xs text-muted-foreground">
          No tasks yet. Create one for a photographer, editor, or anyone else helping with this
          content.
        </div>
      ) : (
        <ul className="space-y-1.5">
          {(tasks as any[]).map((t) => (
            <li
              key={t.id}
              className="flex items-center gap-2 text-sm py-1.5 px-2 rounded border border-border/60 bg-background/40"
            >
        ...
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <ListChecks className="h-4 w-4 text-gold" /> New task for this content
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-3">
            <div>
              ...
              <Input
                ...
                className="mt-1.5"
                placeholder="e.g. Drone footage of property"
              />
            </div>
            <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <Select
                  value={taskOwner || "unassigned"}
                  onValueChange={(v) => setTaskOwner(v === "unassigned" ? "" : v)}
                >
                  <SelectTrigger className="mt-1.5">
                    <SelectValue />
                  </SelectTrigger>
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={create.isPending || !taskTitle.trim()}
